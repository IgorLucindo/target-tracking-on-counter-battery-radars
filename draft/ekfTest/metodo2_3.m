...
while 1
    ...
    gama_array_true = [gama_array_true x_aux(testState)];
    y_true(:, i) = h(x_aux, u);
...
for l = 1:numOfSim
    ...
    for k = 1:numOfFiltering
        ...
        ekf.P(7, 7) = P(7, 7)/1.2^(2*k-2);
    
        [ekf, y_est(:, 1), x_est] = ekf.setInitialState(x_est);
        aux_array = [];
        for i = 2:arrayLength
            aux_array = [aux_array x_est(testState)];
    ...
    state_error = abs((x_est - x_true_last)./x_true_last*100);


    % erro de impacto disparo
    x_aux = x_est;
    while 1
        x_aux = f(x_aux, u);
        if x_aux(3) < p_floor
...
figure
plot(t, gama_array_true(detecThresh + 1:detecThresh + arrayLength) - gama_array_true(detecThresh + 1:detecThresh + arrayLength), 'LineWidth', 2)
for k = 1:numOfFiltering
    hold on
    plot(t(1:end-1), gama_array_avg(2*k-1, 2:end) - gama_array_true(detecThresh + 2:detecThresh + arrayLength), 'LineWidth', 2)
    hold on
    plot(t(1:end-1), gama_array_avg(2*k, 1:end-1) - gama_array_true(detecThresh + 2:detecThresh + arrayLength), 'LineWidth', 2)
