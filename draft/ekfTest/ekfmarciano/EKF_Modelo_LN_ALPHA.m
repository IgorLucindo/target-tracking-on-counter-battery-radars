...
% xE2A = xMM2A + 0*rand(length(xMM2A),1);
% yN2A = yMM2A + 0*rand(length(yMM2A),1);
% zU2A = zMM2A + 0*rand(length(zMM2A),1);
...
alpha_0 =  10^-4;%1.1451e-04;%2*10^-3;%S*C_d0/m;% 5.7256*10^-4;%3.9000e-04;% 10^-6;           % inicialização do parâmetro de arrasto
drag = zeros(1,tam);
p0 = 1.203411;                % densidade local
%p0 = 0.002378;
h = 1.0361*10^-4;    %3.158*10^-5 (1/feet) = 1.0361*10^-4 (1/m) ---- ft = m x 3.28084 
drag(1)= 0;
x_7_Aux=0*wgn(tam,1, 0.0001, 'real');
%lamb(1) =0;
% m = 4.036
% d = 81.03e-3;
% S = pi*(d^2)/4;
...
xk_k(:,1)  = [po; v_(:,1) ; drag(1)];    % Definir estimativa inicial do estado
sig2v= 150^2;       %
%sig2v = var(VelDist)
...
%q_lamb =  (7.25*10^-6)*exp(-0.00005*xk_k(3,k-1));%10^-6; %  1.5600e-04;   1*10^-4

Q_k = [q_x        0          0        0; %10^-8
      0           q_y        0        0;
      0           0          q_z      0;
      0           0          0        q_drag ]; %5*10^-8 %(7.25*10^-13)*exp(-0.00005*xk_k(3,k-1))
...
f43 = - vx*alpha_0*exp(xk_k(7,k))*Pp*h;   % xk_k(7,k) = alpha(k)
f44 = alpha_0*exp(xk_k(7,k))*Pp*(1+(vx/V)^2);
f45 = alpha_0*exp(xk_k(7,k))*Pp*vx*vy/(V^2);
f46 = alpha_0*exp(xk_k(7,k))*Pp*vx*vz/(V^2);
f47 = alpha_0*exp(xk_k(7,k))*vx*Pp;

f53 = - vy*alpha_0*exp(xk_k(7,k))*Pp*h;
f54 = alpha_0*exp(xk_k(7,k))*Pp*vx*vy/(V^2);
f55 = alpha_0*exp(xk_k(7,k))*Pp*(1+(vy/V)^2);
f56 = alpha_0*exp(xk_k(7,k))*Pp*vy*vz/(V^2);
f57 = alpha_0*exp(xk_k(7,k))*vy*Pp;

f63 = - vz*alpha_0*exp(xk_k(7,k))*Pp*h;
f64 = alpha_0*exp(xk_k(7,k))*Pp*vx*vz/(V^2);
f65 = alpha_0*exp(xk_k(7,k))*Pp*vy*vz/(V^2);
f66 = alpha_0*exp(xk_k(7,k))*Pp*(1+(vz/V)^2);
f67 = alpha_0*exp(xk_k(7,k))*vz*Pp;

f73 = 0;   % xk_k(7,k) = alpha(k)
f74 = 0;
f75 = 0;
f76 = 0;
f77 = 0;
...
x_2 = xk_k(5,k);
x_3 = xk_k(6,k);
x_4 = Pp*alpha_0*exp(xk_k(7,k))*x_1;
x_5 = Pp*alpha_0*exp(xk_k(7,k))*x_2;
x_6 = Pp*alpha_0*exp(xk_k(7,k))*x_3 - g;
%x_7 = x_7_Aux(k);
x_7 = 0;
...
drag(k)  = xkm1_km1(7);
Cd_est(k) = (alpha_0*exp(xkm1_km1(7)))*m/(S);
...
subplot(3,2,3:4);
plot(t(1:end),Cd_est(1:end),'r-','Linewidth',1); grid ; hold on;
plot(t(1:end),C_d(1:length(t)),'k-','Linewidth',1); axis([t(1) t(end) 0 0.2])
%title('Coeficiente de Arrasto','Interpreter','latex','FontSize',13);
xlabel('tempo(s)','Interpreter','latex','FontSize',12)
legend('$\hat{C}_{D}(k)$','$C_{D_{REAL}}(k)$','Location','Southeast','Interpreter','latex','FontSize',14); %Southeast  NorthEast
...
subplot(3,2,6);
plot(t(80:end),erro(80:end),'k-','Linewidth',1); grid ; hold on;
stem(t(80),erro(80),'r','Linewidth',1)
title('Erro da estimativa','Interpreter','latex','FontSize',13);
xlabel('tempo(s)','Interpreter','latex','FontSize',12)
legend('$e(t))$','$e(4)$','Location','North East','Interpreter','latex','FontSize',14); %Southeast  NorthEast
exportgraphics(fig5, 'fig_EKF_LN_ALPHA2.pdf', 'ContentType', 'vector');
