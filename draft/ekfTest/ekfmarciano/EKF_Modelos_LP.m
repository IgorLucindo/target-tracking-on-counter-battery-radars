...
ENS=500;
auxENS = zeros(ENS,1);
delta_Tracking = 29; % 35.95  //4seg. de tracking\\TOTAL 23.85 Atenção: o tempo total de rastreio é 23,8s //10%= 2.3850   //20%= 4.7700 //25%= 5.9625// 30%=7.1550  // 35%=8.3475 //40%=9.5400 //45%= 10.7325 // 50%= 11.9250
...
C_Dy = C_D;
C_Dz = C_D;
[x1,y1,z1,vx1,vy1,vz1,vM11,t] = RK4(x1(1), y1(1), z1(1), vxr(1), vyr(1), vzr(1), C_D);
...
x0_range = x0_inicial - delta_x : step_x : x0_inicial + delta_x;
y0_range = y0_inicial - delta_y : step_y : y0_inicial + delta_y;
...
for ix = 1:length(x0_range)
    for iy = 1:length(y0_range)
        for iz = 1:length(z0_range)
            
            for ivx = 1:length(vx0_range)
                for ivy = 1:length(vy0_range)
                      for ivz = 1:length(vz0_range)
...
[xc,yc,zc,t] = RK4(x0_range(ix), y0_range(iy), z0_range(iz), vx0_range(ivx), vy0_range(ivy), vz0_range(ivz),Valpha(end)/2 );  %C_D Valpha(end)/2


% Sincronização da Trajetória
% ==============================================
    % function diferencaQuadratica = calcularDiferenca(trajetoriaCalculada, trajetoriaMedida)
    % trajetoriaCalculada e trajetoria Medida são matrizes onde cada coluna representa [x; y; z]
    %trajetórias são matrizes com 3 linhas (representando x, y, z) 
    %e que as colunas representam diferentes pontos ao longo da trajetória.
...
% figure
% plot3(x1e,y1e,z1e, 'r'); hold on; plot3(xc,yc,zc,'k.'); grid
% xlabel('x (m)','Interpreter','latex','FontSize',14)
% ylabel('y (m)','Interpreter','latex','FontSize',14);
% zlabel('z (m)','Interpreter','latex','FontSize',14);
% legend('EKF','Calculadas ', 'Interpreter', 'latex','Location','northwest','Interpreter','latex','FontSize',12); %%'95\% ellipse (intervalo de confian\c{c}a)',


figure
plot3(xm,ym,zm, 'r'); hold on; plot3(xc,yc,zc,'k.'); grid
xlabel('x (m)','Interpreter','latex','FontSize',14)
ylabel('y (m)','Interpreter','latex','FontSize',14);
zlabel('z (m)','Interpreter','latex','FontSize',14);
legend('EKF','Calculadas ', 'Interpreter', 'latex','Location','northwest','Interpreter','latex','FontSize',12); %%'95\% ellipse (intervalo de confian\c{c}a)',
...
%  figure; surf(X, Y, F); 
% title ('Superfície da Função Custo'); xlabel('X'); ylabel('Y'); zlabel('Função Custo');
%================================================================================================
% % Reorganiza os dados MÉTODO 2
% % Reorganiza os dados
% % Reorganiza os dados
% x = M(1,:);
% y = M(2,:); 
...
% f = M(4,:); 
% 
% % Filtra pontos extremos em F
% mediaF = mean(f);
% desvioPadraoF = std(f);
% limiteSuperior = mediaF + 2 * desvioPadraoF; % Ajuste este fator conforme necessário
% indicesFiltrados = f <= limiteSuperior;
% 
...
Xo_opt=[xMin yMin zMin]
Xo_real=[x1(1) y1(1) z1(1) ]
f_opt = minF
% ...
...
subplot(2,1,1)
surf(X, Y, F); 
title('Superfície da Função Custo', 'FontName', 'Times'); 
xlabel('X', 'FontName', 'Times'); 
ylabel('Y', 'FontName', 'Times'); 
zlabel('Função Custo', 'FontName', 'Times');
...
hold off;
exportgraphics(fig_F_ObjN, 'fig_F_ObjN_.pdf', 'ContentType', 'vector');


% %========================================================================================================
% % Otimização pelo Algortmo de Levenberg-Marquardt
% % Define a função objetivo
% % Esta função deve retornar os resíduos (diferenças nos eixos x, y, z)
% 
% 
% % Configura as opções para usar o algoritmo de Levenberg-Marquardt
% %opcoes = optimoptions('lsqnonlin', 'Algorithm', 'levenberg-marquardt');
% options.Algorithm = 'levenberg-marquardt';
% parametrosIniciais = [xLP, yLP, 0]; % Substitua com um chute inicial apropriado
% % Chama lsqnonlin para minimizar a função custo
% 
...
% z0 = 0;
% 
% %function residuos = funcaoObjetivo(x0, y0, z0)
%     % Calcula xc, yc, zc com base nos parâmetros
%     [xc,yc,zc,t] = RK4(x0, y0, z0, vxr(1), vyr(1), vzr(1),C_D );  %C_D Valpha(end)/2
% tempo = t;
%     % Inicializa o vetor de resíduos
%     residuos = zeros(size(xm, 1) * 3, 1); % Multiplicado por 3 para x, y, z
% 
% 
% % Sincronização da Trajetória
% % ==============================================
...
%      trajetoriaCalculadaAjustada = [trajetoriaCalculadaAjustada(1,1:length(trajetoriaMedida)); trajetoriaCalculadaAjustada(2,1:length(trajetoriaMedida)); trajetoriaCalculadaAjustada(3,1:length(trajetoriaMedida))];
%   end
%     numPontosMedida2 = size(trajetoriaMedida, 2);
%     numPontosCalculada2 = size(trajetoriaCalculadaAjustada, 2);
% 
%     xm = trajetoriaMedida(1,:);
%     ym = trajetoriaMedida(2,:);
%     zm = trajetoriaMedida(3,:);
...
% 
% 
%     % Calcula os resíduos ponderados
%     for k = 1:size(xm, 1)
%         residuos((k-1)*3 + 1) = (xm(k) - xc(k)) / sigma_x(k);
%         residuos((k-1)*3 + 2) = (ym(k) - yc(k)) / sigma_y(k);
%         residuos((k-1)*3 + 3) = (zm(k) - zc(k)) / sigma_z(k);
%     end
% 
%     [parametrosOtimizados, resnorm] = lsqnonlin(@funcaoObjetivo, parametrosIniciais, [], [], options);
% %end
...
[xPL,yPL,zPL,t] = RK4(xMin, yMin, 0, Vx(1), Vy(1), Vz(1),Valpha(end)/2 ); 


plot3(xPL,yPL,zPL,'b--','linewidth',1); hold on;
%colorbar;       % Adiciona uma barra de cores para indicar a relação com o tempo
cb = colorbar;%('northoutside');  % Coloca a barra de cores na parte superior e na horizontal;  % Adiciona uma barra de cores
cb.Label.Interpreter = 'latex';
cb.Label.String = 'Tempo (s)';  % Legenda da barra de cores
