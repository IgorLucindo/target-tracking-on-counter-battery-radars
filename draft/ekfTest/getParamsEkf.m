...
function [f, h, F, H, f_rev, F_rev] = getParamsEkf(T)
    ...
    function F = innerF(x, u)
        ...
        F = [1, 0, 0, T - (T^2*gama*v)/4 - (T^2*gama*vx^2)/(4*v), -(T^2*gama*vx*vy)/(4*v), -(T^2*gama*vx*vz)/(4*v), -(T^2*vx*v)/4;
             ...
             0, 0, 0, (T^2*gama^2*h_*vx*vz)/(4*v), (T^2*gama^2*h_*vy*vz)/(4*v), (T^2*(2*gama*vz*h_^2 + gama*((gama*v)/2 + (gama*vz^2)/(2*v))*h_))/2 - T*gama*h_, (T^2*(h_*((gama*vz*v)/2 + g) + h_^2*vz^2 + (gama*h_*vz*v)/2))/2 - T*h_*vz + 1];
    end
